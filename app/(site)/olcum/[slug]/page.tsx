// /olcum/<slug> — yalnız type: "benchmark" (BRIEF §2.2). İnce rota: veriyi
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (icerik === null || icerik.type !== "benchmark") return {};
...
export default async function OlcumSayfasi({ params }: Props) {
  const { slug } = await params;
  const icerik = await yayindakiIcerik(slug);
  if (icerik === null || icerik.type !== "benchmark") {
    // Slug değişmişse redirects koleksiyonu kalıcı yönlendirir (BRIEF §2.2)
    const hedef = await guvenliYonlendirmeBul(`/olcum/${slug}`);
