// /olcum — ölçüm/kıyaslama (type: "benchmark") arşivi (BRIEF §2.2). Desen: app/(site)/makale/page.tsx.
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  let bos = true;
  try {
    bos = (await turSayisi("benchmark")) === 0;
  } catch {
    bos = true;
  }
  return turArsiviUstVerisi("benchmark", sayfaOku(ham["sayfa"]), { bos });
}

export default async function OlcumArsivi({ searchParams }: Props) {
  const ham = await searchParams;
  return <TurArsivi tur="benchmark" sayfa={sayfaOku(ham["sayfa"])} />;
}
