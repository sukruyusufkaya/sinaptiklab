...
import { testBul } from '@/lib/icerik/ogrenme';
...
export default async function OgGorseli({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const k = await testBul(slug);
  if (!k) return ogKarti({ etiket: 'TEST', baslik: 'Sinaptik Lab' });

  return ogKarti({
    etiket: 'TEST',
    baslik: k.ad,
    altMetin: k.ozet,
    kunye: [`${k.soruSayisi} soru`, `${k.dakika} dk`],
  });
}
