...
import { arastirmaBul } from '@/lib/icerik/arastirma';

/**
 * ARAŞTIRMA paylaşım kartı.
 *
...
export default async function OgGorseli({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const k = await arastirmaBul(slug);
  if (!k) return ogKarti({ etiket: 'ARAŞTIRMA', baslik: 'Sinaptik Lab' });
