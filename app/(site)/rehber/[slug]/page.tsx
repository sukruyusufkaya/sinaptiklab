// /rehber/<slug> — yalnız type: "guide" (BRIEF §2.2). İnce rota: veriyi
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (icerik === null || icerik.type !== "guide") return {};
...
export default async function RehberSayfasi({ params }: Props) {
  const { slug } = await params;
  const icerik = await yayindakiIcerik(slug);
  if (icerik === null || icerik.type !== "guide") {
    // Slug değişmişse redirects koleksiyonu kalıcı yönlendirir (BRIEF §2.2)
    const hedef = await guvenliYonlendirmeBul(`/rehber/${slug}`);
