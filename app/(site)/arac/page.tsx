// /arac — araç ve model kartı (type: "tool") arşivi (BRIEF §2.2). Desen: app/(site)/makale/page.tsx.
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  let bos = true;
  try {
    bos = (await turSayisi("tool")) === 0;
  } catch {
    bos = true;
  }
  return turArsiviUstVerisi("tool", sayfaOku(ham["sayfa"]), { bos });
}

export default async function AracArsivi({ searchParams }: Props) {
  const ham = await searchParams;
  return <TurArsivi tur="tool" sayfa={sayfaOku(ham["sayfa"])} />;
}
