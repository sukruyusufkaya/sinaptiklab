// /uygulama/<slug> — yalnız type: "tutorial" (BRIEF §2.2). İnce rota: veriyi
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (icerik === null || icerik.type !== "tutorial") return {};
...
export default async function UygulamaSayfasi({ params }: Props) {
  const { slug } = await params;
  const icerik = await yayindakiIcerik(slug);
  if (icerik === null || icerik.type !== "tutorial") {
    // Slug değişmişse redirects koleksiyonu kalıcı yönlendirir (BRIEF §2.2)
    const hedef = await guvenliYonlendirmeBul(`/uygulama/${slug}`);
