// /laboratuvar — laboratuvar (type: "lab") arşivi (BRIEF §2.2). Desen: app/(site)/makale/page.tsx.
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  let bos = true;
  try {
    bos = (await turSayisi("lab")) === 0;
  } catch {
    bos = true;
  }
  return turArsiviUstVerisi("lab", sayfaOku(ham["sayfa"]), { bos });
}

export default async function LaboratuvarArsivi({ searchParams }: Props) {
  const ham = await searchParams;
  return <TurArsivi tur="lab" sayfa={sayfaOku(ham["sayfa"])} />;
}
