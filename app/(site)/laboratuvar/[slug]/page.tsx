// /laboratuvar/<slug> — yalnız type: "lab" (BRIEF §2.2). İnce rota: veriyi
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (icerik === null || icerik.type !== "lab") return {};
...
export default async function LaboratuvarSayfasi({ params }: Props) {
  const { slug } = await params;
  const icerik = await yayindakiIcerik(slug);
  if (icerik === null || icerik.type !== "lab") {
    // Slug değişmişse redirects koleksiyonu kalıcı yönlendirir (BRIEF §2.2)
    const hedef = await guvenliYonlendirmeBul(`/laboratuvar/${slug}`);
