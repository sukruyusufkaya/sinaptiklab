...
import { analizBul } from '@/lib/icerik/gundem';

/**
 * ANALİZ paylaşım kartı.
 *
...
export default async function OgGorseli({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const k = await analizBul(slug);
  if (!k) return ogKarti({ etiket: 'ANALİZ', baslik: 'Sinaptik Lab' });

  return ogKarti({
    etiket: 'ANALİZ',
    baslik: k.baslik,
    altMetin: k.girizgah,
    kunye: [k.tarih, `${k.okumaDakika} dk`],
  });
}
