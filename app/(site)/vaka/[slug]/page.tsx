// /vaka/<slug> — yalnız type: "case" (BRIEF §2.2). İnce rota: veriyi
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (icerik === null || icerik.type !== "case") return {};
...
export default async function VakaSayfasi({ params }: Props) {
  const { slug } = await params;
  const icerik = await yayindakiIcerik(slug);
  if (icerik === null || icerik.type !== "case") {
    // Slug değişmişse redirects koleksiyonu kalıcı yönlendirir (BRIEF §2.2)
    const hedef = await guvenliYonlendirmeBul(`/vaka/${slug}`);
