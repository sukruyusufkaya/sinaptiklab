...
export async function IcerikSayfasi({ icerik }: { icerik: IcerikDetayDTO }) {
  const { icerik: govde } = await mdxDerle(icerik.body);
  ...
  return (
    <article className="mx-auto max-w-[1280px] px-[var(--gutter)] py-10">
      ...
      {/* 7 — gövde (Faz 2 taban tipografisi; tam okuma düzeni Faz 3) */}
      <div className="mt-10 max-w-[var(--govde-olcu)] [&_blockquote]:mt-4 [&_blockquote]:border-l-2 [&_blockquote]:border-doku [&_blockquote]:pl-4 [&_blockquote]:text-murekkep-2 [&_h2]:mt-12 [&_h2]:scroll-mt-6 [&_h2]:text-2xl [&_h2]:font-bold [&_h3]:mt-8 [&_h3]:scroll-mt-6 [&_h3]:text-xl [&_h3]:font-semibold [&_hr]:my-8 [&_hr]:border-doku [&_li]:mt-1 [&_ol]:mt-4 [&_ol]:list-decimal [&_ol]:pl-5 [&_p]:mt-4 [&_table]:w-full [&_table]:border-collapse [&_table]:text-sm [&_td]:border [&_td]:border-doku [&_td]:px-3 [&_td]:py-2 [&_th]:border [&_th]:border-doku [&_th]:px-3 [&_th]:py-2 [&_th]:text-left [&_ul]:mt-4 [&_ul]:list-disc [&_ul]:pl-5">
        {govde}
      </div>
